...
public class QuestionsAboutImportantTest extends AbstractWebTest {
    ...
    public static Object[][] generateData() {
        ...
        return new Object[][]{
                ...
                {"Как рассчитывается время аренды?", "Допустим, вы оформляете заказ на 8 мая. Мы привозим самокат 8 мая в течение дня. Отсчёт времени аренды " +
                ...
                {"Вы привозите зарядку вместе с самокатом?", "Самокат приезжает к вам с полной зарядкой. Этого хватает на восемь суток — даже если будете кататься " +
    ...
    public void checkQuestionsAboutImportantTest2(String input, String expected) {

        String actualResult = mainPage.collectAnswersInQLTwo(input);

        System.out.println(String.format("Сравниваем ожидаемый результат = %s с фактическим = %s", expected, actualResult));
